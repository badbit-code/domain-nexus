...
def add_to_db(file_name,json_contents):
	...
	with sqlite3.connect(db_path,detect_types=sqlite3.PARSE_DECLTYPES) as conn, closing(conn.cursor()) as cur:
		cur.execute(create_query)
		cur.executemany(f'insert or ignore into godaddy_details (domain_name, available, domain_length) values (?,?,?)', (i+[len(i[0].split('.')[0])] for i in data)) 
		
	conn.close()

with FTP('ftp.godaddy.com','auctions') as ftp:
	files_needed=(x for x in ftp.nlst() if x.startswith('all_listings') and not x.startswith('all_listings_'))
