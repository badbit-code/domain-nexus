...
async def db_ops(db_path, table_name, session):
    async with aiosqlite.connect(db_path) as conn, conn.execute(
        f"select * from {table_name} where date_added = ?", (today,)
    ) as cur:

        async with conn.execute(
            f"select count(domain_name) from {table_name} where date_added = ?",
            (today,),
        ) as cur_:
        ...
        async for row in cur:
            for column_name, result in zip(
                columns, await asyncio.create_task(fetch_details(row[0], session))
            ):
                await conn.execute(
                    f"update {table_name} set {column_name}=(?) where domain_name=(?)",
                    (result, row[0]),
                )
                await conn.commit()
            ...
            for column_name_tld, exists, creation_date in await whois(row[0]):
                if exists:
                    await conn.execute(
                        f"update {table_name} set {column_name_tld}=(?) where domain_name=(?)",
                        (True, row[0]),
                    )

                    if column_name_tld == row[0].split(".", 1)[-1].lower():
                        await conn.execute(
                            f"update {table_name} set creation_date=(?) where domain_name=(?)",
                            (creation_date, row[0]),
                        )
...
async def main():
    db_table_pairs = [
        ("db/__godaddy_db.db", "godaddy_details"),
        ("db/__sedo_db.db", "sedo_details"),
    ]
