...
def spaces_upload(
    ...
    region_name="sfo2",
    bucket="downloads.tldquery",
):
    spaces_session = session.Session(access_id, secret_key, region_name=region_name)
    resource = spaces_session.resource(
        "s3", endpoint_url="https://sfo2.digitaloceanspaces.com"
    )
    try:
...
def upload(files):
    with spaces_upload() as space:
        for source, target in files:
            space.upload_fileobj(
                df2io(source), target, ExtraArgs={"ACL": "public-read"}
            )
...
def gen_csv():
    ...
    with spaces_upload() as spaces, FTP(
        "ftp.mcdanieltechnologies.com", "datamover@tldquery.com", "7T5sUnu2dQ$g"
    ) as ftp:
        ...
        data_gen = (
            (base_url.format(i.key), i.last_modified.date(), i.size)
            for i in spaces.objects.all()
