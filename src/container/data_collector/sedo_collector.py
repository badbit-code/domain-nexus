...
cur.execute(
    """create table if not exists sedo_details
...
next(
    file_like
)  # "Domain Name";"Start Time";"End Time";"Reserve Price";"Domain is IDN";"Domain has hyphen";"Domain has numbers";"Domain Length";TLD;Traffic;Link
for idx, row in enumerate(csv.reader(file_like, delimiter=";")):
    if limiter is not None and idx == limiter:
        break  # limiter for testing
    row_ = row[0], row[2], row[7]
    cur.execute(
        "insert or ignore into sedo_details (domain_name, available, domain_length) values (?,?,?)",
        row_,
    )
