...
with closing(sqlite3.connect("db/__godaddy_db.db")) as conn, closing(
    conn.cursor()
) as cur:
    cur.execute('select tbl_name from sqlite_master where type="table"')
    godaddy_df = pd.concat(
        pd.read_sql_query(
            f"select * from {table_name} where date_added=?", con=conn, params=(today,)
        )
        for table_name, *_ in cur
    )

with closing(sqlite3.connect("db/__sedo_db.db")) as conn, closing(conn.cursor()) as cur:
    sedo_df = pd.read_sql_query(
        "select * from sedo_details where date_added=?", con=conn, params=(today,)
    )

godaddy_df["Source"] = "Godaddy"
sedo_df["Source"] = "Sedo"

combined_df = pd.concat((godaddy_df, sedo_df)).drop_duplicates(
    "domain_name"
)  # combine and remove duplicate rows based on domain_name column
...
creation_date_report = current_report[
    (today_ - current_report["Creation Date"]).dt.days >= 730
]

make_name = (
    lambda x: f"""{x}{'_' if x else ''}{datetime.today().strftime("%Y-%m-%d")}.csv"""
)  # should I make a folder or just name with `x`_report_name?

dfs = current_report, combined_df, length_report, archive_report, creation_date_report
target_names = [
    "currentreport.csv",
    *(make_name(x) for x in ("", "length", "archive", "creation_date")),
]
