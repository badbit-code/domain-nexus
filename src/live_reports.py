    ...
    while True:
        try:
            response = requests.get(
                f"https://web.archive.org/cdx/search/cdx?url={domain_name}&output=json&fl=statuscode"
            ).text
...
currency_map = {
    "&#163": currency_converter(1.34),
    "EUR": currency_converter(1.21),
    "$US": currency_converter(1),
}
...
df = pd.DataFrame(
    res, columns=["Domain Name", "Cost", "Archive Count", "Last Updated", "Buy"]
)
...
upload(
    source_dest
)  # or factory('upload', '/wp-content/uploads/2020/11/reportfolder')(source_dest) :p
